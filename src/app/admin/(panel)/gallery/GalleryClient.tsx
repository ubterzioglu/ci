 ...
 * public site. Photos can be edited inline or removed. State is optimistic-ish:
 * the server action returns the new/updated row and we reconcile local state.
 */
  ...
  return (
    <div className="space-y-6">
      ...
      <AdminSurface
        ...
        description={`${items.length} fotoğraf — alt metin zorunlu, açıklama (sol alt köşe yazısı) en fazla ${CAPTION_MAX_LENGTH} karakter ve isteğe bağlıdır.`}
      >
        {items.length === 0 ? (
          <AdminEmptyState
            title="Henüz fotoğraf yok"
            description="Yukarıdaki formdan ilk Atmosfer fotoğrafını yükleyin."
          />
        ) : (
...
function UploadForm({ onCreated, onError }: UploadFormProps) {
  ...
  return (
    <AdminSurface
      title="Yeni Fotoğraf"
      description="Bir görsel seçin, görme engelliler için alt metni girin ve isterseniz kısa bir açıklama ekleyin."
    >
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="block">
            ...
            <input
              ...
              required
              maxLength={300}
              placeholder="Örn. Bahçede şarap ve mevsim tabağı"
        ...
        <label className="block">
          <span className="mb-1.5 flex items-center justify-between font-body text-sm font-semibold text-charcoal">
            <span>Açıklama (sol alt köşe yazısı, isteğe bağlı)</span>
...
function GalleryCard({ item, onSaved, onDeleted, confirm, toast }: GalleryCardProps) {
  ...
  function handleSave() {
    startTransition(async () => {
      const result = await updateGalleryItemAction(item.id, { alt, caption });
      if (result.ok) {
  ...
  function handleDelete() {
    startTransition(async () => {
      const ok = await confirm({
        title: 'Fotoğrafı sil',
        description: 'Bu fotoğraf galeriden ve depolamadan kalıcı olarak silinecek.',
        confirmLabel: 'Sil',
        destructive: true,
      });
      if (!ok) return;
        ...
        {editing ? (
          ...
          <>
            ...
            <p className="font-body text-xs text-muted">
              {item.caption ? `Açıklama: ${item.caption}` : 'Açıklama yok'}
            </p>
            <div className="flex gap-2">
              <button
                type="button"
